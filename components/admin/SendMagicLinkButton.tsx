...
export function SendMagicLinkButton({ email, clientName }: { email: string; clientName: string }) {
  ...
  return (
    <div className="min-w-0">
      ...
      {state === 'done' && (
        <div className="mt-3 p-4 rounded-xl bg-[#EEEDF8] border border-[#C5C4E0] max-w-full overflow-hidden">
          <p className="text-xs font-medium text-green-700 mb-2">✓ Invite email sent to {email}</p>
          <p className="text-xs text-[#5A5575] mb-2">Copy link as backup:</p>
